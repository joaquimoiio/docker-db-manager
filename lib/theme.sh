...
# ─── Banners ASCII por banco (estilo ANSI-Shadow, mesma família do banner DB Manager) ──
# Cada função imprime 6 linhas coloridas. Mantém largura ≤ 76 colunas.

db_banner_postgres() {
    local c=$'\033[38;5;75m' c2=$'\033[38;5;39m'
    printf '%s' "$c"
    cat <<'EOF'
    ██████╗  ██████╗ ███████╗████████╗ ██████╗ ██████╗ ███████╗███████╗
    ██╔══██╗██╔═══██╗██╔════╝╚══██╔══╝██╔════╝ ██╔══██╗██╔════╝██╔════╝
...
# Dispatch — chama o banner correto para o banco.
db_banner() {
    case "$1" in
        postgres)  db_banner_postgres  ;;
        mysql)     db_banner_mysql     ;;
        oracle)    db_banner_oracle    ;;
        sqlserver) db_banner_sqlserver ;;
    esac
}
